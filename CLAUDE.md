...
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│   model.py                         FreeCAD Document (.FCStd)         │
│   ────────                         ─────────────────────────         │
│   Python script that               "Compiled" 3D view of the         │
│   defines the 3D model             design for visualization          │
│                                                                      │
│   • Single source of truth         • Generated by executing          │
│   • Version controllable             model.py                        │
...
└── MyProject/                   # Project folder (named after document)
    ├── model.py                 # 3D model code (source of truth)
...
**Project files:**
- **model.py** - Python script that generates the 3D model. Claude edits this directly.
...
User: "Add a 10mm hole to the center"
    ...
    │                       │
    ▼                       ▼
Read model.py          Edit model.py
(understand design)    (add hole code)
                            │
                            ▼
                Execute modified model.py
                            │
                            ▼
...
2. **Claude Code reads** - Understands current design from `model.py`
...
- **Full history** - `model.py` shows how the design evolved over time
- **Git-friendly** - Diff, branch, merge, and review design changes
- **Reproducible** - Run `model.py` on any machine to recreate the design
...
│   ├── source.py              # Manages model.py lifecycle, backup/restore
...
Maintains `model.py` alongside the FreeCAD document:
...
- `get_source_path()` - Returns `{doc_dir}/{doc_stem}/model.py`

#### Claude Code Integration

`backends/claude_code.py` invokes the `claude` CLI with:
- `--allowedTools Read,Glob,Grep,Edit` - Can read and edit model.py
- `--output-format stream-json` - Streaming responses
- Working directory set to project folder

System prompt instructs Claude to:
1. Read model.py to understand current design
2. Use Edit tool to modify model.py directly
