...
Function InitUser
  ...
  # Get Buildable language
  
  ReadRegStr $FCLangName SHELL_CONTEXT "${APP_REGKEY_SETUP}" "Buildable Language"
...
Function PostMultiUserPageInit
  # check if this Buildable version is already installed
  ReadRegStr $0 SHCTX "${APP_UNINST_KEY}" "UninstallString"
  ${if} $0 != ""
   # check if the uninstaller was accidentally deleted
   # if so, don't bother the user if they really want to install a new Buildable over an existing one
   ...
   # installing over an existing installation of the same Buildable release is not necessary
   # if the users does this, they most probably have a problem with Buildable that can better be solved
   # by reinstalling Buildable
  ...
  # check if there is an existing Buildable installation of the same Buildable series
...
Function .onInit
  ...
  # Check that Buildable is not currently running
  ${nsProcess::FindProcess} ${BIN_BUILDABLE} $R0
...
Function un.onInit
  ...
  # Macro to investigate name of Buildable's preferences folders to be able remove them
  ...
  # abort when Buildable was installed with admin permissions but the user doesn't have administrator privileges
  ...
  ${andif} $MultiUser.Privileges != "Power"
   ...
   Abort
  ${endif}
  # warning when Buildable couldn't be found in the registry
  ${if} $0 == "" # check in HKCU
  ...
  # question message if the user really wants to uninstall Buildable
