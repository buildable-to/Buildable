...
Provides just enough information for AI to understand existing designs
without overwhelming the prompt with verbose data.
"""
...
def _get_selection_details() -> str:
    """Get detailed selection info including sub-elements."""
    try:
...
def _describe_object_lean(obj) -> str:
    """One-line summary: label, type, key dimensions, position if non-origin."""
    type_short = obj.TypeId.split("::")[-1]
    parts = [f"  {obj.Label} ({type_short})"]

    # Key dimensions from properties
    dim_parts = []
    for prop_name in ("Length", "Width", "Height", "Radius", "Diameter"):
        try:
    ...
    if not dim_parts and hasattr(obj, "Shape"):
        try:
