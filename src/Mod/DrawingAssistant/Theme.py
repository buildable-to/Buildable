...
"""
Theme - Centralized design tokens for the Drawing Assistant UI.
...
def set_theme(dark=True):
    ...
    FreeCAD.Console.PrintMessage(
        f"DrawingAssistant: Theme set to {'dark' if dark else 'light'}\n"
    )
    for cb in _theme_change_callbacks:
        try:
            cb()
        except Exception as e:
            FreeCAD.Console.PrintWarning(f"DrawingAssistant: Theme callback error: {e}\n")
