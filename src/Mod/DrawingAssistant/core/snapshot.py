...
def get_snapshots_dir() -> Optional[Path]:
    ...
    snapshots_dir = config_dir / "DrawingAssistant" / "snapshots"
    snapshots_dir.mkdir(parents=True, exist_ok=True)
    return snapshots_dir
...
def save_snapshot(include_brep: bool = True) -> tuple:
    ...
    if not snapshot:
        FreeCAD.Console.PrintWarning("DrawingAssistant: No active document for snapshot\n")
        return (None, None)

    snapshots_dir = get_snapshots_dir()
    if not snapshots_dir:
        FreeCAD.Console.PrintWarning("DrawingAssistant: Could not determine snapshots directory\n")
        return (None, None)
    ...
    try:
        ...
        FreeCAD.Console.PrintMessage(f"DrawingAssistant: Snapshot saved to {json_path}\n")
    except Exception as e:
        FreeCAD.Console.PrintError(f"DrawingAssistant: Failed to save snapshot JSON: {e}\n")
        return (None, None)
